  ...
  try {
    ...
    if (!isSupabaseConfigured()) {
      const filteredTrips = trips.filter((trip) =>
        ...
        trip.subtitle.toLowerCase().includes(searchTerm) ||
        trip.region.toLowerCase().includes(searchTerm) ||
    ...
    const { data, error } = await supabase
      .from('trips')
      .select('id, title, subtitle, region, price, price_value, image, tags')
      .eq('available', true)
      .or(`title.ilike.%${searchTerm}%,subtitle.ilike.%${searchTerm}%,region.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%`)
      .limit(limit);
    
    if (error) {
      return NextResponse.json(
        { error: 'Error searching trips', details: error.message },
        { status: 500 }
      );
    }
