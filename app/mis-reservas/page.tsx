...
type PageState = 'loading' | 'login' | 'magic_link_sent' | 'authenticated';
...
function MisReservasContent() {
  ...
  }, [supabase.auth, fetchBookings, searchParams]);
  ...
  const handleMagicLink = async () => {
    ...
    try {
      ...
      const { error: otpError } = await supabase.auth.signInWithOtp({
        email: email.trim(),
        options: {
          emailRedirectTo: `${siteUrl}/auth/callback?next=/mis-reservas`,
        },
      });
  ...
  const handleGoogleLogin = async () => {
    ...
    const { error: oauthError } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${siteUrl}/auth/callback?next=/mis-reservas`,
      },
    });
          ...
          <div className="max-w-md mx-auto">
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
              ...
              <p className="text-sm text-gray-500 mb-6">
                Haz clic en el enlace del email para acceder a tus reservas. El enlace expira en 1 hora.
              </p>
              <Button
                variant="outline"
                onClick={() => {
